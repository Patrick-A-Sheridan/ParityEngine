...
import graphing_calculator_052.Tokenizer.Token;
...
public abstract class BasicMathUtil {
    ...
    for (int i = 0; i < input.length(); i++) {
        ...
        if (i < input.length() - 1) {
            ...
            // Trigger '*' if: DigitBeforeParen, ParenBeforeDigit, DigitBeforeVar, OR VarBeforeParen
            if ((Character.isDigit(current) && next == '(') || 
                (current == ')' && Character.isDigit(next)) ||
                (Character.isDigit(current) && isVariable(next)) ||
                (isVariable(current) && next == '(')) {
                end.append("*");
            }
        }
    }
